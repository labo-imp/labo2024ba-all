...
#He agregado 5 hiperparametros; y hace una OPTIMIZACION BAYESIANA
# entre los rangos definidos
...
hs <- makeParamSet(
  ...
  makeIntegerParam("num_leaves", lower = 8L, upper = 1024L), # Número máximo de hojas en un árbol
  makeNumericParam("feature_fraction", lower = 0.1, upper = 1.0), # Fracción de características utilizadas en cada iteración
  makeIntegerParam("min_data_in_leaf", lower = 1L, upper = 8000L), # Mínimo número de datos en una hoja
  makeIntegerParam("envios", lower = 5000L, upper = 15000L), # Número de envíos (este parámetro parece específico a tu problema y no estándar en LightGBM)
  
  # Regularización
  makeNumericParam("lambda_l1", lower = 0.0, upper = 1.0), # Regularización L1 para evitar sobreajuste
  makeNumericParam("lambda_l2", lower = 0.0, upper = 1.0), # Regularización L2 para evitar sobreajuste
  
  # Ganancia y muestreo
  makeNumericParam("min_gain_to_split", lower = 0.0, upper = 0.1), # Ganancia mínima requerida para dividir un nodo
  makeNumericParam("bagging_fraction", lower = 0.5, upper = 1.0), # Fracción de datos utilizados en el muestreo por iteración
  makeIntegerParam("bagging_freq", lower = 1L, upper = 10L), # Frecuencia de bagging
  
  # Otros parámetros
  makeNumericParam("min_split_gain", lower = 0.0, upper = 1.0), # Ganancia mínima para realizar una división
  makeIntegerParam("n_estimators", lower = 100, upper = 10000), # Número de estimadores (árboles)
  makeNumericParam("subsample", lower = 0.5, upper = 1.0), # Fracción de muestras utilizadas para entrenar cada árbol
  makeNumericParam("colsample_bytree", lower = 0.5, upper = 1.0), # Fracción de características utilizadas para cada árbol
  makeNumericParam("scale_pos_weight", lower = 1.0, upper = 10.0), # Peso de la clase positiva en problemas desbalanceados
  makeIntegerParam("max_bin", lower = 255L, upper = 1000L), # Número máximo de bins para la discretización de características
  makeNumericParam("drop_rate", lower = 0.0, upper = 1.0), # Tasa de abandono (dropout) para evitar sobreajuste

  
  # Parámetro adicional
  makeIntegerParam("max_depth", lower = -1L, upper = 20L) # Profundidad máxima de los árboles (-1 para sin límite)
