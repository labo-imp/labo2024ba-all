...
dataset[, ganancia := ifelse(clase_ternaria == "BAJA+2", 237000, -3000)]

# para testing agrego la probabilidad
dataset[fold == 2, prob_baja2 := prediccion[, "BAJA+2"]]

# calculo la ganancia en testing  qu es fold==2
ganancia_test <- dataset[fold == 2 & prob_baja2 > z, sum(ganancia)]

# escalo la ganancia como si fuera todo el dataset
ganancia_test_normalizada <- ganancia_test / (( 100 - PARAM$training_pct ) / 100 )

estimulos <- dataset[fold == 2 & prob_baja2 > z, .N]
aciertos <- dataset[fold == 2 & prob_baja2 > z & clase_ternaria == "BAJA+2", .N]
