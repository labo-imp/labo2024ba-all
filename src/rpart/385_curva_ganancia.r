...
gra <- ggplot(
  ...
  aes(x = pos, y = ganancia_acumulada,
      color = factor(ifelse(fold == 1, "train", "test")))
) +
  geom_line() +
  ggtitle(paste0("Curva de Ganancia para minsplit=", PARAM$minsplit,
                 ", minbucket=", PARAM$minbucket, 
                 ", maxdepth=", PARAM$maxdepth)) +
  labs(caption = paste0("Train gan max: ", 
                        dataset[fold==1, max(ganancia_acumulada)],
                        " Test  gan max: ", 
                        dataset[fold==2, max(ganancia_acumulada)])) +
  theme(plot.caption = element_text(hjust = 1, vjust = 1, color = "blue"))
