...
particionar <- function(data, division, agrupa = "", campo = "fold",
                        start = 1, seed = NA) {
       if (!is.na(seed)) set.seed(seed) # setea la seed si no la tiene

       bloque <- unlist(mapply( #unlist convierte una lista en un vector # mapply esfuncion 
         #que aplica una funcion determinada a ciertos vectores. EN este caso genera una funcion nueva que repite el elemento "y" unas "x" veces al
         #parametro "division" y a una sequencia de 1 a la longitud del dataset).
              function(x, y) {
                     rep(y, x)
              },
       ...
       data[, (campo) := sample(rep( ## usando datatable, agarra el dataset (completo) y crea el campo fold aka pliegue
              bloque, ## el vector que generamos arriba repitiendo el punto
              ceiling(.N / length(bloque)) ## cuantas veces hay que repetir bloque para tener suficientes elementos.
       ))[1:.N],
       by = agrupa ##  Indica que esta operación se realizará por grupos. agrupa es la columna o columnas que definen cómo se agrupan las filas de data.
       ]
}
#------------------------------------------------------------------------------
#------------------------------------------------------------------------------
