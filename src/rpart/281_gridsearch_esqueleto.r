...
PARAM$semilla_primigenia <- 113287
PARAM$qsemillas <- 20
...
for (vcp in c(-0.5, -0.4, -0.3, -0.2, -0.1, 0)) {
  for (vminbucket in c(4)) {
    # notar como se agrega

    # vminsplit  minima cantidad de registros en un nodo para hacer el spl80it
    param_basicos <- list(
      "cp" = vcp, # complejidad minima
      "maxdepth" = 9, # profundidad máxima del arbol
      "minsplit" = 1100, # tamaño minimo de nodo para hacer split
