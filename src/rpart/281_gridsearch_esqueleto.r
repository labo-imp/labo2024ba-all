...
PARAM$semilla_primigenia <- 610559
PARAM$qsemillas <- 20
...
for (vmax_depth in c(4, 6, 8, 10, 12, 14)) {
  print(paste("Iterando vmax_depth:", vmax_depth))
  ...
  for (vmin_split in c(1000, 800, 600, 400, 200, 100, 50, 20, 10)) {
    ...
    for (vmin_bucket in c(500, 400, 300, 200, 100, 50, 20, 10, 5)){
      ...
      for (vcp in c(-1, -0.5, 0, 0.001, 0.01)) {
        ...
        # Print all parameters and the obtained result
        print(paste("Parametros - vmax_depth:", vmax_depth,
                    ", vmin_split:", vmin_split,
                    ", vmin_bucket:", vmin_bucket,
                    ", vcp:", vcp,
                    ", Ganancia:", ganancias[[1]]$ganancia_test))
        
      } # Fin del loop de vcp
    } # Fin del loop de vmin_bucket
  } # Fin del loop de vmin_split

  # grabo cada vez TODA la tabla en el loop mas externo
  fwrite( tb_grid_search_detalle,
          file = "gridsearch_detalle.txt",
          sep = "\t" )
} # Fin del loop de vmax_depth
